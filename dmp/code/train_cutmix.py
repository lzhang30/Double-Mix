...
def apply_cutout(images, labels = None, num_holes=1, max_h_size=50, max_w_size=50, max_d_size=50):
    
    """
    Apply Cutout to a batch of images.

    :param images: Tensor of images, shape (b, c, d, w, h)
    :param num_holes: Number of holes to cut out from each image
    :param max_h_size: Maximum height of the hole
    :param max_w_size: Maximum width of the hole
    :param max_d_size: Maximum depth of the hole
    :return: Tensor of images with Cutout applied
    """
    b, c, d, w, h = images.shape
    mask = np.ones((b, c, d, w, h), dtype=np.float32)

    for i in range(b):
        for _ in range(num_holes):
            w_size = np.random.randint(1, max_w_size)
            h_size = np.random.randint(1, max_h_size)
            d_size = np.random.randint(1, max_d_size)

            w_start = np.random.randint(0, w)
            h_start = np.random.randint(0, h)
            d_start = np.random.randint(0, d)

            w_end = np.clip(w_start + w_size, 0, w)
            h_end = np.clip(h_start + h_size, 0, h)
            d_end = np.clip(d_start + d_size, 0, d)

            mask[i, :, d_start:d_end, w_start:w_end, h_start:h_end] = 0
    images = images * torch.from_numpy(mask).to(images.device)
    if labels != None:
        
        
        labels = labels * torch.from_numpy(mask).to(labels.device).long()
...
if __name__ == '__main__':
    ...
    average_per_channel_total = torch.ones(config.num_cls).cuda() 
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            image_l,label_l = apply_cutout(image_l, label_l)
