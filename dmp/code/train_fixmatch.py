...
def cutmix_3d(image1, label1, image2, label2):
    assert image1.size() == image2.size()
    assert label1.size() == label2.size()

    B, C, D, W, H = image1.size()
    
    dx, dw, dh = random.randrange(D), random.randrange(W), random.randrange(H)

    rd, rw, rh = random.randint(1, D - dx), random.randint(1, W - dw), random.randint(1, H - dh)

    output_image = image1.clone()
    output_label = label1.clone()

    output_image[:, :, dx:dx+rd, dw:dw+rw, dh:dh+rh] = image2[:, :, dx:dx+rd, dw:dw+rw, dh:dh+rh]
    output_label[:, dx:dx+rd, dw:dw+rw, dh:dh+rh] = label2[:, dx:dx+rd, dw:dw+rw, dh:dh+rh]
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            if args.mixed_precision:
                with autocast():
                    ...
                    softmax_A_u = torch.softmax(output_A, dim=1)
                    max_A_u = torch.argmax(softmax_A_u, dim=1, keepdim=True).long()
                    confidence_A = torch.max(softmax_A_u, dim=1)[0]
                    mask_A = confidence_A > thr
                    filtered_max_A_u = max_A_u * mask_A.unsqueeze(1)

                    softmax_B_u = torch.softmax(output_B, dim=1)
                    max_B_u = torch.argmax(softmax_B_u, dim=1, keepdim=True).long()
                    #print(f'max_B_u {max_B_u.shape}')
                    confidence_B = torch.max(softmax_B_u, dim=1)[0]
                    mask_B = confidence_B > thr
                    
                    filtered_max_B_u = max_B_u * mask_B.unsqueeze(1)
                    
                    #print(f'mask_B {mask_B.shape}')
                    #print(filtered_max_A_u.shape)   
                    ...
                    loss_sup = loss_func_A(output_A_l, label_l) + \
                        loss_func_B(output_B_l, label_l)
                    loss_cps = cps_loss_func_A(output_A, filtered_max_B_u) +\
                        cps_loss_func_B(output_B, filtered_max_A_u)
